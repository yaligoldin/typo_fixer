...
class TypoFixer:
    ...
    @staticmethod
    def _fix_line(content_lines: List[str], typo_to_correct: FixedTypo) -> List[str]:
        line_to_correct = content_lines[typo_to_correct.typo_line - 1]
        line_to_correct.replace(typo_to_correct.typo_word, typo_to_correct.typo_correction)
        content_lines[typo_to_correct.typo_line - 1] = line_to_correct
        return content_lines

    def _create_fixed_file(self, file_to_fix: File, typos_corrections: List[FixedTypo]) -> File:
    ...
    def create_fixed_file(self, typos: FileTypo) -> None:
        ...
        fixed_file = self._create_fixed_file(typos.file, fixed_typos)
